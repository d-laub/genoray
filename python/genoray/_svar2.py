        ...
        **Same meaning as** :meth:`from_vcf`'s `max_mem` -- both pipelines
        ...
        with write_reporting(progress, log_level) as (rx, level):
            return _core.run_pgen_conversion_pipeline(
                ...
                dosage_fields=dosage_field_tuples,
                readers=readers,
                dosage_readers=dosage_readers,
        ...
        **`None` (the default) means a DETECTED budget** -- same 80%-of-cgroup
        ...
        with write_reporting(progress, log_level) as (rx, level):
            return _core.run_vcf_list_conversion_pipeline(
                ...
                fields=FieldSpec(info=info, format=format_),
                # No `max_mem_bytes`: the merge path plans by core count only,
                # and passing one is rejected rather than ignored (#200).
                plan=PlanSettings(
                    chunk_size=chunk_size,
                    max_threads=threads,  # None => auto
                    long_allele_capacity=long_allele_capacity,
                ),
                contig_membership=contig_membership,
                ploidy=ploidy,
                skip_out_of_scope=skip_out_of_scope,
                signatures=signatures,
                check_ref=check_ref,
        ...
        *,
        ...
        with write_reporting(progress, log_level) as (rx, level):
            return _core.run_svar1_conversion_pipeline(
                ...
                # No `max_mem_bytes`: this path plans by core count only, and
                # passing one is rejected rather than ignored (#200).
                plan=PlanSettings(
                    chunk_size=chunk_size,
                    max_threads=threads,  # None => auto
                    long_allele_capacity=long_allele_capacity,
                ),
                contig_starts=starts,
                contig_lens=lens,
                pos_per_contig=pos_pc,
                ref_bytes_per_contig=ref_bytes_pc,
                ref_offsets_per_contig=ref_off_pc,
                alt_bytes_per_contig=alt_bytes_pc,
                alt_offsets_per_contig=alt_off_pc,
                format_fields=format_tuples,
                format_src_dtypes=src_dtypes,
