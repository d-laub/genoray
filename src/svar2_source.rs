...
// Re-exported (not just imported) because existing integration tests reach
// these through `genoray_core::svar2_source::*` — see svar2_view.rs for the
// canonical definitions. `read_n_samples` stays crate-internal (it was
// `pub(crate)` before the move, and no test reaches it via this path).
pub(crate) use crate::svar2_view::read_n_samples;
pub use crate::svar2_view::{OverlapMode, keeps, query_window};
