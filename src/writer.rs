...
pub fn run_io_writer(rx_sparse: Receiver<SparseChunk>, output_dir: &str) {
    ...
    while let Ok(chunk) = rx_sparse.recv() {
        ...
        // buffered writers
        let mut pos_file =
            BufWriter::new(File::create(&pos_path).expect("Failed to create pos.bin"));
        let mut key_file =
            BufWriter::new(File::create(&key_path).expect("Failed to create key.bin"));
        ...
        pos_file
            .write_all(pos_bytes)
            .expect("Failed to write chunk positions");
        key_file
            .write_all(key_bytes)
            .expect("Failed to write chunk keys");
